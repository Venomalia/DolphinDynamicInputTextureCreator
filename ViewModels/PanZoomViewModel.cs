...
namespace DolphinDynamicInputTextureCreator.ViewModels
{
    public class PanZoomViewModel : Other.PropertyChangedBase
    {
        ...
        public void FillRegion()
        {
            ...
            //one fill region is enough per key
            foreach (Data.RectRegion r in InputPack.EditingTexture.Regions)
            {
                if (r.Device == InputPack.SelectedRegionBrush.SelectedEmulatedDevice &&
                    r.Key == InputPack.SelectedRegionBrush.SelectedEmulatedKey &&
                    r.X == f.X & r.Y == f.Y & r.Width == f.Width & r.Height == f.Height)
                {
                    return;
                }
            }
